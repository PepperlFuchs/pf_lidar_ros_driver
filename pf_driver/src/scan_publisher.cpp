...
void ScanPublisher::read(PFR2000Packet &packet)
{
    header_publisher_.publish(packet.header);
    to_msg_queue<PFR2000Packet>(packet);
}

void ScanPublisher::read(PFR2300Packet &packet)
{
    // header_publisher_.publish(packet.header);
    to_msg_queue<PFR2300Packet>(packet);
}
...
void ScanPublisher::to_msg_queue(T &packet)
{
    ...
    if(d_queue_.empty())
    {
        ...
        if(d_queue_.size() > 100)
            d_queue_.pop_front();
    }
    if(packet.header.header.packet_number == 1)
    {
        // publish_scan(msg);   //TODO comment out the next line instead
        queue_.try_enqueue(std::move(d_queue_.front()));
        d_queue_.pop_front();
    ...
    // prev_num_packet_ += packet.header.num_points_packet;
}
